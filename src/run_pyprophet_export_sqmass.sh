...
#SBATCH --mem-per-cpu=32000Mb       # memory per cpu - match with treads argument when running
...
lib=""
output="merged.osw"
while getopts ":h:?:f:a:l:o:s:p:" opt; do
    case "$opt" in
    h|\?)
        showhelp
        exit 0
...
exportCommand="apptainer exec --cleanenv --bind ${TMPDIR}:/mnt --pwd /mnt ${sig} pyprophet export parquet --in=${osw} --out=${osw}_xic.parquet --pqpfile $(basename ${pqpfile})"
echo $exportCommand
$exportCommand
...
chown -R $user:def-hroest $TMPDIR/${osw}_xic.parquet # !!changeit!!
mv $TMPDIR/${osw}_xic.parquet ./ # !!changeit!!
