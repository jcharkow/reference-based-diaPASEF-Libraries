...
#SBATCH --mem-per-cpu=80000Mb       # memory per cpu - match with treads argument when running
...
scoreCommand="apptainer exec --cleanenv --bind ${TMPDIR}:/mnt --pwd /mnt ${sig} pyprophet score --in=${osw} --level=ms1ms2 --threads=1 ${additionalParam}"
echo ${scoreCommand}
$scoreCommand


peptideCommand="apptainer exec --cleanenv --bind ${TMPDIR}:/mnt --pwd /mnt ${sig} pyprophet infer peptide --in=${osw} --context=global" 
echo ${peptideCommand}
$peptideCommand


proteinCommand="apptainer exec --cleanenv --bind ${TMPDIR}:/mnt --pwd /mnt ${sig} pyprophet infer protein --in=${osw} --context=global" 
echo ${proteinCommand}
$proteinCommand

libCommand="apptainer exec --cleanenv --bind ${TMPDIR}:/mnt --pwd /mnt ${sig} pyprophet export library --in ${osw} --out ${osw/.oswpq}_lib.tsv"
echo ${libCommand} 
$libCommand

# library only filtering
libCommand="apptainer exec --cleanenv --bind ${TMPDIR}:/mnt --pwd /mnt ${sig} pyprophet export library --in ${osw} --out ${osw/.oswpq}_lib_onlyFilter.tsv --no-rt_calibration --no-intensity_calibration --no-im_calibration"
echo ${libCommand} 
$libCommand
