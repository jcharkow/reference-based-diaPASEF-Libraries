...
# this script uses the .TSV loader and produces a .PQP output
# It also accepts a zstd compressed tsv file and will decompress on requireed node
...
ckFileSz $nonlin_irt_lib "nonlinear_irt transition file"
touch $output
...
RTNORM="-RTNormalization:alignmentMethod lowess -RTNormalization:lowess:span 0.01 -RTNormalization:NrRTBins 8 -RTNormalization:MinBinsFilled 4"
...
SCORING="-Scoring:TransitionGroupPicker:min_peak_width 5.0 -Scoring:TransitionGroupPicker:PeakPickerChromatogram:sgolay_frame_length 11 -Scoring:stop_report_after_feature 5 -Scoring:Scores:use_ion_mobility_scores"
...
runcommand="apptainer exec --bind $TMPDIR:/mnt --pwd /mnt ${sig} OpenSwathWorkflow \
    ...
    -tr "$(basename ${library})" \
    -tr_irt $(basename ${irt_lib}) \
    -tr_irt_nonlinear $(basename ${nonlin_irt_lib}) \
    -out_features ${output}.osw $OUTCHROM \
    $ADVANCED \
    $SWATH_WIN \
    $CALIBRATION \
