            ...
            if top_replicate is not None:
                ...
                for lib_dir in lib_dirs:
                    ...
                    if f"{lib_dilution}pg_" in lib_dir and f"PyDIA_{top_replicate}_" in lib_dir:
                        ...
                        lib_file = os.path.join(osw_lib_path, lib_dir, "pyprophet_SVM", f"{lib_dir}{suffix}")
...
def get_library_path(lib_id, base_lib_path, use_only_filter=True):
    ...
    suffix = "_lib_onlyFilter.tsv" if use_only_filter else "_lib.tsv"
    lib_file = os.path.join(base_lib_path, "osw", lib_id, "pyprophet_SVM", f"{lib_id}{suffix}")
...
def generate_irt_libraries(scripts_path, irt_precs_path, pqp_libraries_dir, available_libraries, use_only_filter=True, use_sbatch=False):
    ...
    for irt_precs_file in irt_precursor_files:
        ...
        out_name = f"2025-07-25-{dilution}pg-{irt_type}-PeptDeepNoMods_{filter_suffix}_rep{replicate}.tsv"
...
def process_openswath_workflow(use_sbatch=True, use_only_filter=True):
    ...
    base_lib_path = os.path.abspath(os.path.join(main_dir, "../PeptDeep-NoMods-Analysis/"))
    ...
    sig_prophet = os.path.abspath(os.path.join(main_dir, "../../bin/sif/2025-09-18-pyprophet.sif"))
    ...
    for dilution in sorted(samples_by_dilution.keys()):
        ...
        for mzml_file, output_basename in samples_by_dilution[dilution]:
            ...
            for lib_id in library_ids:
                ...
                filter_suffix = "onlyFilter" if use_only_filter else "refined"
                irt_lin_path = os.path.join(irt_folder_path, f"2025-07-25-{lib_dilution}pg-linIrt-PeptDeepNoMods_{filter_suffix}_rep{lib_replicate}.tsv")
                irt_nonlin_path = os.path.join(irt_folder_path, f"2025-07-25-{lib_dilution}pg-nonLinIrt-PeptDeepNoMods_{filter_suffix}_rep{lib_replicate}.tsv")
                ...
                if use_only_filter:
                    additional_param = "-irt_im_extraction_window 0.2 -ion_mobility_window 0.06 -rt_extraction_window 500"
                else:
                    additional_param = "-irt_im_extraction_window 0.2 -ion_mobility_window 0.06 -rt_extraction_window 300"

                ##### special runs:
                if (not use_only_filter) and (lib_dilution == 2500) and (sample_dilution == 100):
                    print("SPECIAL!!")
                    print("Lib dilution", lib_dilution)
                    print("Sample Replicate", sample_replicate)
                    print("Sample Dilution", sample_dilution)
                    irt_lin_path=os.path.join(os.path.dirname(irt_lin_path), "2025-07-25-100pg-linIrt-PeptDeep_refined_for_2500pg_rep_1_lib.tsv")
                    irt_nonlin_path=os.path.join(os.path.dirname(irt_lin_path), "2025-07-25-100pg-nonLinIrt-PeptDeep_refined_for_2500pg_rep_1_lib.tsv")

                if (not use_only_filter) and (lib_dilution == 2500) and (sample_dilution == 250) and (sample_replicate == 8):
                    print("SPECIAL!!")
                    print("Lib dilution", lib_dilution)
                    print("Sample Replicate", sample_replicate)
                    print("Sample Dilution", sample_dilution)
                    irt_lin_path=os.path.join(os.path.dirname(irt_lin_path), "2025-07-25-250pg-linIrt-PeptDeep_refined_for_2500pg_rep_1_lib.tsv")
                    irt_nonlin_path=os.path.join(os.path.dirname(irt_lin_path), "2025-07-25-250pg-nonLinIrt-PeptDeep_refined_for_2500pg_rep_1_lib.tsv")
                ...
                if os.path.isdir(oswpq_dir_path):
                    ...
                    pyprophet_args_lda = "--classifier=SVM --ss_scale_features"
