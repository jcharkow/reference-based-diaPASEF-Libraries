...
libPath="../K562-Bruker-Library-Analysis/formatted/"
irtNonLin=${libPath}/2025-03-06-nonlinear-irt-in-silico.tsv
lib=${libPath}/_ip2_ip2_data_paser_spectral_library__Bruker_Human_6Frags_decoys.pqp
...
do
	...
	checkAndRunSbatch ${output}.osw ${fromMain}/../../development/2025-06-23-PanHuman-Library-Full-New-Workflow/run_osw_tsv.sh ${fromMain}/${mzml} ${fromMain}/${lib} ${fromMain}/${irtNonLin} ${fromMain}/${irtNonLin} ${output} True ${fromMain}/${sigOSW} $additionalParam
...
then
	...
	checkAndRunSbatch merged.oswpqd ${fromMain}/${scripts}/run_pyprophet_parquet_multiple.sh -o merged.osw -a '--classifier=LDA' -s ${fromMain}/${sigProphet} \
		-f ../$(basename ${m400/.mzML})/oswOut/$(basename ${m400/.mzML}).oswpq \
		-f ../$(basename ${m460/.mzML})/oswOut/$(basename ${m460/.mzML}).oswpq \
		-f ../$(basename ${m520/.mzML})/oswOut/$(basename ${m520/.mzML}).oswpq \
		-f ../$(basename ${m580/.mzML})/oswOut/$(basename ${m580/.mzML}).oswpq \
...
libPrefix=2025-07-10-osw-bruker-gpf-refined
...
origLib=${fromMain}/${libPath}/_ip2_ip2_data_paser_spectral_library__Bruker_Human.tsv
...
then
	...
	libOut=2025-08-14-bruker-lib-refined_GPF_manyFrags_OSW.tsv 
	module load apptainer
