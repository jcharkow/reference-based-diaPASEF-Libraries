...
libPath="../K562-PanHuman-Analysis/formattedLib/"
irtNonLin=${libPath}/2025-03-06-nonLin-iRT.tsv
lib=../PanHuman-Library-Creation/phl004_s32_imAppended_fixed_with_decoys.pqp # This library has correct fragment ion annotations
...
do
	...
	checkAndRunSbatch ${output}.osw.zst ${fromMain}/../../development/2025-06-23-PanHuman-Library-Full-New-Workflow/run_osw_tsv.sh ${fromMain}/${mzml} ${fromMain}/${lib} ${fromMain}/${irtNonLin} ${fromMain}/${irtNonLin} ${output} True ${fromMain}/${sigOSW} $additionalParam
...
then
	...
	checkAndRunSbatch merged.oswpqd ${fromMain}/${scripts}/run_pyprophet_parquet_multiple.sh -o merged.osw -a '--classifier=LDA' -s ${fromMain}/${sifProphet} \
		-f ../$(basename ${m400/.mzML})/oswOut/$(basename ${m400/.mzML}).oswpq \
		-f ../$(basename ${m460/.mzML})/oswOut/$(basename ${m460/.mzML}).oswpq \
		-f ../$(basename ${m520/.mzML})/oswOut/$(basename ${m520/.mzML}).oswpq \
		-f ../$(basename ${m580/.mzML})/oswOut/$(basename ${m580/.mzML}).oswpq \
...
libPrefix=2025-07-11-osw-PHL-gpf-refined
...
libPrefix=2025-09-19-osw-PHL-gpf-refined-SVM
if [[ -d pyprophet_SVM/merged.oswpqd ]]
then

	fromMain="../"
	checkAndRun ${libPrefix}.tsv apptainer exec ${fromMain}/${sifProphet2} pyprophet --log-level debug export library --in pyprophet_XGB/merged.oswpqd --out ${libPrefix}.tsv
	checkAndRun ${libPrefix}_onlyFilter.tsv apptainer exec ${fromMain}/${sifProphet2} pyprophet export library --in pyprophet_XGB/merged.oswpqd --out ${libPrefix}_onlyFilter.tsv --no-rt_calibration --no-intensity_calibration --no-im_calibration
	checkAndRun ${libPrefix}_nativeRT.tsv apptainer exec ${fromMain}/${sifProphet2} pyprophet export library --in pyprophet_XGB/merged.oswpqd --out ${libPrefix}_nativeRT.tsv --rt_unit RT
fi
cd ..

#####################################
# Here use predicted fragments on mars specifically run notebook 2025-09-19-Create-PHL-Extra-Fragments.ipynb
# ###################################
checkCreateFolder osw_xtraFrags

###########################################
# Run OpenSwathAssayGenerator
##########################################
baseLib=2025-09-19-PHL-xtraFrags-fixed.tsv
module load apptainer
checkAndRun ${baseLib/.tsv/_formatted.tsv} apptainer exec --bind $(pwd):/mnt --pwd /mnt ${fromMain}/${sigOSWFull} OpenSwathAssayGenerator -in ${baseLib} -out ${baseLib/.tsv/_formatted.tsv} -min_transitions 6 -max_transitions 20

##################################
# Convert to .pqp
#################################
checkAndRun ${baseLib/.tsv/_formatted.pqp} apptainer exec --bind $(pwd):/mnt --pwd /mnt ${fromMain}/${sigOSWFull} TargetedFileConverter -in ${baseLib/.tsv/_formatted.tsv} -out ${baseLib/.tsv/_formatted.pqp}
...
do
	...
	checkAndRunSbatch ${output}.osw ${fromMain}/${scripts}/run_osw_nativeRT.sh ${fromMain}/${mzml} ${fromMain}/osw_xtraFrags/${lib} ${output} False ${fromMain}/${sigOSWFull} $additionalParam

	if [[ -f ${output}.osw ]]
	then
		checkAndRunSbatch ${output}.oswpq ${fromMain}/${scripts}/run_pyprophet_export_parquet_scoring.sh -f ${output}.osw -s ${fromMain}/${sifProphet2}
	fi
...
then
	...
	libOut=2025-09-19-PHL-lib-refined_GPF_manyFrags_OSW.tsv 
	module load apptainer
