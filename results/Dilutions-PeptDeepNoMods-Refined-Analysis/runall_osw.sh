...
function run_osw_pyprophet() { 
	...
	then
		...
		checkAndRunSbatch ${output}.sqMass_xic.parquet ${fromMain}/${scripts}/run_pyprophet_export_sqmass.sh -f ${output}.sqMass -p ${output}.osw -s ${fromMain}/${sigProphetSqMass}
	fi
...
libPth=../Dilutions-PeptDeep-NoMods-Lib/osw
fromMain="../.."
for lib in $(find ${fromMain}/${libPth} -wholename "*/pyprophet_*/*_lib.tsv")
do

	dilution_lib=$(basename ${lib%ng_DIA_Slot*lib.tsv})
	dilution_lib=${dilution_lib#*100nL-}
	baseLib=$(basename ${lib})

	echo dilution is ${dilution_lib}
	found=False

	if [[ ${dilution_lib} == 100 || ${dilution_lib} == 25 ]]
	then
		if [[ ${lib} == *"pyprophet_XGB"* ]]
		then
...
for irt in $(find ${fromMain}/${libPth} -wholename "*/2025-01-21*.tsv")
do
	echo ${irt}
	if [[ ${irt} == *"2025-01-21-1ng"* ]]
	then
		continue
	else
		dilution_irt=${irt%ng*}
