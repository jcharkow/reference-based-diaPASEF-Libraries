...
for m in $m1 $m2 $m3
do
	runName=$(basename -s .mzML ${m}) # the output file path 
	checkCreateFolder ${runName}
	fromMain="../../"
	baseLib=library.tsv
	...
	checkAndRun ${baseLib} cp ${fromMain}/${runName}/${baseLib} ${baseLib}
...
do
	...
	checkAndRunSbatch ${output}.osw ${fromMain}/${scripts}/run_osw.sh ${fromMain}/${mzml} ${fromMain}/formattedLib/${output}/library_osw_6Frags_decoys.pqp ${fromMain}/formattedLib/${output}/2025-03-06-linear-irt-for-diaTracer.tsv ${fromMain}/formattedLib/${output}/2025-03-06-nonlinear-irt-for-diaTracer.tsv ${output} False ${fromMain}/${sigOSW} $additionalParam
