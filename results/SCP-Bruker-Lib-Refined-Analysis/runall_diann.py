...
def get_appropriate_libraries(sample_info, onlyFilter=False):
    """
    ...
    libs_out = []
    if dilution < 100:
        libs_out.append("2025-06-17-Refine-BrukerLib-100pg-rep3")
    if dilution < 250:
        libs_out.append("2025-06-17-Refine-BrukerLib-250pg-rep1")
    if dilution < 500:
        libs_out.append("2025-06-17-Refine-BrukerLib-500pg-rep3")
    if dilution < 1000:
        libs_out.append("2025-06-17-Refine-BrukerLib-1000pg-rep3")
    if dilution < 2500:
        libs_out.append("2025-06-17-Refine-BrukerLib-2500pg-rep2")
    if dilution < 5000:
        libs_out.append("2025-06-17-Refine-BrukerLib-5000pg-rep2")

    if onlyFilter:
        libs_out = [ i + '-onlyFilter' for i in libs_out ]
...
def process_samples(use_qvalue_one=False, use_sbatch=True, onlyFilter=False):
    """
    ...
    folder_name = 'diann'
    if use_qvalue_one:
        folder_name = folder_name + '_full'
    if onlyFilter:
        folder_name = folder_name + '_onlyFilter'
    ...
    for dilution, dilution_samples in samples.items():
        ...
        for sample in dilution_samples:
            ...
            lib_identifiers = get_appropriate_libraries(sample, onlyFilter=onlyFilter)
...
def main():
    ...
    process_samples(use_qvalue_one=False, use_sbatch=use_sbatch, onlyFilter=False)
    process_samples(use_qvalue_one=False, use_sbatch=use_sbatch, onlyFilter=True)
    
    # Run with qvalue=1.0
    process_samples(use_qvalue_one=True, use_sbatch=use_sbatch, onlyFilter=False)
